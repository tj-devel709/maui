...
namespace Microsoft.Maui.Controls
{
	...
	public sealed class PinchGestureRecognizer : GestureRecognizer, IPinchGestureController
	{
		...
		internal void SendPinch(Element sender, double delta, Point currentScalePoint, object recognizer)
		{
			EventHandler<PinchGestureUpdatedEventArgs> handler = PinchUpdated;
			if (handler != null)
			{
				handler(sender, new PinchGestureUpdatedEventArgs(GestureStatus.Running, delta, currentScalePoint, recognizer));
			}
		...
		internal void SendPinchCanceled(Element sender, object recognizer)
		{
			EventHandler<PinchGestureUpdatedEventArgs> handler = PinchUpdated;
			if (handler != null)
			{
				handler(sender, new PinchGestureUpdatedEventArgs(GestureStatus.Canceled, recognizer));
			}
		...
		internal void SendPinchEnded(Element sender, object recognizer)
		{
			EventHandler<PinchGestureUpdatedEventArgs> handler = PinchUpdated;
			if (handler != null)
			{
				handler(sender, new PinchGestureUpdatedEventArgs(GestureStatus.Completed, recognizer));
			}
		...
		internal void SendPinchStarted(Element sender, Point initialScalePoint, object recognizer)
		{
			EventHandler<PinchGestureUpdatedEventArgs> handler = PinchUpdated;
			if (handler != null)
			{
				handler(sender, new PinchGestureUpdatedEventArgs(GestureStatus.Started, 1, initialScalePoint, recognizer));
			}
